...
while getopts "n:e:b" opt; do
  case "$opt" in
  n)
    NUM_VALIDATORS="$OPTARG"
    FLAG_SPECIFIED=true
    ;;
  e)
    EPOCH_DURATION_MS="$OPTARG"
    FLAG_SPECIFIED=true
    ;;
  b)
    BENCHMARK_MODE=true
    ...
    ;;
  *)
    echo "Usage: $0 [-n num_validators] [-e epoch_duration_ms] [-b]"
    exit 1
    ;;
  esac
done
...
EOF
    ...
    if [[ "$BENCHMARK_MODE" == "true" ]]; then
      # Deterministic benchmark gas accounts generated by GenesisConfig::benchmark_gas_keys()
      # with seed 0. These addresses are stable and match the keys in benchmark.keystore.
      cat >>"$GENESIS_TEMPLATE" <<EOF
...
generate_benchmark_keystore() {
  ...
  if [[ -f "$BENCH_TEMP_DIR/benchmark.keystore" ]]; then
    ...
    echo ""
    echo "To run the stress tool against this network (from a checkout of"
    echo "https://github.com/iotaledger/network-benchmark):"
