...
/// A `DenyRuleProposal` whose declared authority matches `author` is recorded
/// through the consensus pipeline and its rules become active for the next
/// commit; a proposal whose author does not match the consensus block author
/// is dropped by `verify_consensus_transaction`.
#[tokio::test]
async fn deny_rule_proposal_through_consensus_updates_active_set() {
    use iota_sdk_types::Address;
    use iota_types::deny_rule_governance::{DenyRuleConfig, DenyRuleProposal, DenyRuleSet};
    ...
    let sequenced = SequencedConsensusTransaction {
        ...
        transaction: crate::consensus_handler::SequencedConsensusTransactionKind::External(
            ConsensusTransaction::new_deny_rule_proposal(proposal),
        ),
    };
    epoch_store
        .process_consensus_transactions_for_tests(
    ...
    let active = epoch_store.get_active_deny_rules();
    ...
    let sequenced = SequencedConsensusTransaction {
        ...
        consensus_index: Default::default(),
        transaction: crate::consensus_handler::SequencedConsensusTransactionKind::External(
            ConsensusTransaction::new_deny_rule_proposal(forged),
        ),
    };
    epoch_store
        .process_consensus_transactions_for_tests(
...
async fn deny_rule_proposal_ignored_when_flag_disabled() {
    use iota_sdk_types::Address;
    use iota_types::deny_rule_governance::{DenyRuleConfig, DenyRuleProposal};
