...
#[test]
fn compute_active_deny_rules_applies_stake_thresholds() {
    ...
    // Two members deny `active_address`; one of them also flips two switches.
    proposals.insert(
        names[0],
        deny_proposal(
            names[0],
...
/// `update_active_deny_rules` recomputes from the recorded proposals and
/// reports whether the active set changed. A newer full-state proposal that
/// omits an entry withdraws that vote, deactivating the entry
/// (drop-to-remove).
#[tokio::test]
async fn update_active_deny_rules_drops_withdrawn_rules() {
    let authority_state = TestAuthorityBuilder::new().build().await;
    let store = authority_state.epoch_store_for_testing();
    ...
    assert_eq!(*store.get_active_deny_rules(), DenyRuleSet::default());
    assert!(!store.update_active_deny_rules().unwrap());
    ...
    assert!(store.update_active_deny_rules().unwrap());
    let active = store.get_active_deny_rules();
...
async fn restart_seed_restores_persisted_deny_rules() {
    ...
    let active = reopened.get_active_deny_rules();
    assert!(active.denied_addresses.contains(&address));
    assert!(active.user_transaction_disabled);
}
