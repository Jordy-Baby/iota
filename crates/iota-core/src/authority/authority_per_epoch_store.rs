...
pub struct AuthorityPerEpochStore {
    ...
    /// deny rule proposals. Recomputed via `update_active_deny_rules` and
    /// seeded from the `deny_rule_proposals` table on construction.
    active_deny_rules: ArcSwap<DenyRuleSet>,
...
impl AuthorityPerEpochStore {
    ...
    pub fn new(
        ...
        let active_deny_rules = ArcSwap::from_pointee(Self::compute_active_deny_rules(
    ...
    pub fn should_record_deny_rule_proposal(&self, proposal: &DenyRuleProposal) -> bool {
        ...
        if stale {
            debug!(
                "Ignoring deny rule proposal from {:?} with stale generation {}",
    ...
    pub fn get_active_deny_rules(&self) -> Arc<DenyRuleSet> {
        self.active_deny_rules.load_full()
    }
    ...
    pub(crate) fn compute_active_deny_rules(
        proposals: &BTreeMap<AuthorityName, DenyRuleProposal>,
        committee: &Committee,
    ) -> DenyRuleSet {
    ...
    pub(crate) async fn process_consensus_transactions<C: CheckpointServiceNotify>(
        ...
        let verified_non_randomness_transactions: Vec<_> =
        ...
        match &transaction {
            ...
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                if !self.protocol_config().calculate_validator_scores() {
                    ...
                    return Ok(ConsensusTransactionResult::ConsensusMessage);
                }
                if !self
                    .get_reconfig_state_read_lock_guard()
                    .should_accept_consensus_certs()
