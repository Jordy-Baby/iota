...
impl CoreSignals {
    pub fn new(context: Arc<Context>) -> (Self, CoreSignalsReceivers) {
        ...
        // buffer here will not reduce memory usage significantly. Keep headroom
        // for a full proposal burst so lagging subscribers don't drop blocks.
        let capacity = (context.parameters.dag_state_cached_rounds as usize)
            .max(2 * context.parameters.block_rate_burst() as usize);
