...
//!
//! Selection sorts healthy candidates by effective latency and draws each
//! position with probability proportional to the inverse latency from a window
//! of the best remaining peers — the selection shape of the checkpoint
//! state-sync's peer balancer, with a weighted draw inside the window. Peers
//! whose most recent fetch failed always come last, outside the randomization.
//! So fast peers lead in proportion to how fast they are, the window keeps a
//! large slow tail from diluting the weights, and a just-failed peer cannot
//! displace a healthy one. An exploration fraction of selections ignores
...
/// Each position in the selection order is drawn from at most this many of the
/// best-ranked remaining peers. Bounding the draw to a small window keeps the
/// inverse-latency weights meaningful (a large slow tail cannot dilute them)
/// and matches the checkpoint state-sync's `PEER_BALANCER_SELECTION_WINDOW`,
/// whose sorted-then-sample-from-the-window selection is proven in production.
...
impl PeerResponsiveness {
    ...
    ///
    /// Non-failed candidates are sorted by effective latency and each position
    /// is then drawn with probability proportional to `1 / effective_latency`
    /// from a window of the [`SELECTION_WINDOW`] best remaining peers — the
    /// same selection shape as the checkpoint state-sync's peer balancer, with
    /// a weighted draw inside the window. A peer whose most recent fetch
    /// failed does not take part in the randomization at all: it is ordered
    /// behind every non-failed candidate (fastest of the failed first) until
    /// its next success. A fraction of calls (see [`EXPLORE_PROBABILITY`])
    /// ignore ranking and return a uniform shuffle. `rng` is consumed fresh on
    /// every call.
    pub(crate) fn prioritize<R: Rng>(
        ...
        // an all-equal set, e.g. cold start, is ordered uniformly), then each
        // position is drawn with probability proportional to `1 / latency`
        // from the window of the best remaining peers. Peers whose most recent
        // fetch failed always go last, fastest first, and re-enter the
        // randomized draw with their next success.
        ...
        while !healthy.is_empty() {
            let window = healthy.len().min(SELECTION_WINDOW);
            let total: f64 = healthy[..window]
                .iter()
                .map(|(latency, _, _)| 1.0 / latency)
                .sum();
            let mut draw = rng.gen::<f64>() * total;
            let mut chosen = window - 1;
            for (i, (latency, _, _)) in healthy[..window].iter().enumerate() {
                draw -= 1.0 / latency;
                if draw <= 0.0 {
                    chosen = i;
                    break;
